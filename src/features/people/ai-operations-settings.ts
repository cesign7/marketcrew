...
> = {
  moa: {
    roleModel: "낯선 가설은 바로 결재하지 않고 검증된 근거만 안건으로 승격합니다. 처음 실행하는 조정 유형은 대표 첫 승인을 받은 뒤에만 위임 후보로 바꿉니다.",
    responsibility: "하위 캐릭터의 자유 탐색 가설과 근거 요청을 비용, 근거, 실행 위험, 대표 위임 가능성 기준으로 정리합니다.",
    outputContract: "가설, 확인된 사실, 추가 근거 요청, 대표 첫 승인 필요 여부, 다음부터 위임 가능한 조건을 분리해 보고합니다.",
    monthlyReviewRule: "월말에는 승인된 가설, 반려된 가설, 위임된 규칙의 성과와 보고 누락 여부를 비교해 다음 달 탐색 범위를 조정합니다.",
  },
  gro: {
    roleModel: "규칙 엔진이 잡은 저성과 키워드를 먼저 처리하고, 정해진 지표 밖의 키워드·기기·시간대 조합도 자유 탐색합니다. 저성과라도 시즌·브랜드·학습 기간상 유지해야 할 수 있음을 먼저 확인합니다.",
    responsibility: "네이버 키워드광고의 낮은 전환율, 주문 없는 클릭, 높은 CPA, 기기/시간대 차이를 키워드별 업무카드로 만들고 필요한 광고 설정 근거를 요청합니다.",
    outputContract: "키워드, 기기, 시간대별 조정 이유, 유지 예외 사유, 예상 효과, 예산 상한, 중단 기준, 대표 첫 승인 또는 위임 후보 여부를 함께 보고합니다.",
    monthlyReviewRule: "월별로 자유 탐색 키워드의 승인률, 유지 판단 적중률, 광고비 대비 성과, 위임 규칙의 중단 기준 적중률을 확인합니다.",
  },
  pro: {
  ...
  maru: {
    roleModel: "브랜드별 커머스 운영 흐름에서 예상 못 한 주문/상품/판매채널 문제를 찾고, 서로 다른 브랜드를 직접 비교하지 않습니다.",
    responsibility: "스티커씨와 커피프린트를 분리해 주문, 매출, 상품 흐름, 판매채널 준비 상태, 운영 비용 근거를 확인합니다.",
    outputContract: "브랜드, 판매채널, 상품 흐름, 확인할 운영 근거, 모아 처리 또는 대표 결재 필요 여부를 함께 제시합니다.",
    monthlyReviewRule: "월별 자유 탐색 안건이 브랜드별 주문 안정성, 판매채널 준비, 비용 대비 성과로 이어졌는지 점검합니다.",
  },
  day: {
    roleModel: "LLM이 제안한 근거 후보와 규칙 엔진의 광고 성과 판정을 실제 원천 필드와 집계 기준으로 검증합니다. 전환 추적이 불확실한 키워드는 조정 전에 반드시 보류시킵니다.",
    responsibility: "수집 누락, 전환 추적, 주문 연결, 전년동기, 음력 명절 비교, 세그먼트, 원천 필드 후보를 확인해 가설 신뢰도를 매깁니다.",
    outputContract: "확인된 근거, 부족한 근거, 추가 수집 요청, 판단 보류 사유, 조정 전 확인해야 할 추적/주문 연결 상태를 명확히 남깁니다.",
