...
export async function proxyRequestToBackend(request: Request, pathOverride?: string, options: BackendProxyOptions = {}) {
  if (isBackendRuntime()) {
    return undefined;
  }
...
export function isBackendRuntime(env: NodeJS.ProcessEnv = process.env) {
  return env.MARKETCREW_BACKEND_MODE === "1" || env.RAILWAY_SERVICE_NAME === "marketcrew-api";
}

export function isHostedFrontendRuntime(env: NodeJS.ProcessEnv = process.env) {
  return env.VERCEL === "1" && !isBackendRuntime(env);
}

function getBackendApiUrl(env: NodeJS.ProcessEnv = process.env) {
...
function getBackendProxyTimeoutMs(env: NodeJS.ProcessEnv = process.env, overrideMs?: number) {
