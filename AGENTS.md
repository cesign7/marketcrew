...
| 키 | 이름 | 역할 |
|----|------|------|
| `opi` | 모아 | AI Chief of Staff / 마케팅 운영실장. 하위 보고를 대표 결재 안건으로 종합한다. |
...
화면 표시명과 문서상 캐릭터명은 `모아`로 통일한다. 기존 저장 데이터와 코드 호환을 위해 내부 키 `opi`, 실행 타입 `opi_planner`, 도메인 타입 `OpiSynthesisReport`는 유지한다.
