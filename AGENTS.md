...
- 다음 권장 작업: 1차 MVP는 완료로 닫고, 이후 작업은 운영 배포 후 `/api/operations/provider-sync`에서 `검색광고 성과 N건`과 `쇼핑검색광고 성과 N건`이 실제로 생성되는지 확인하고, 다음 확장으로 광고그룹 설정 스냅샷, normalized analytics schema, 실제 provider write executor 설계를 진행한다. 실제 LLM call은 비용 가드와 원천 행 제외 계약을 통과해야 하며, provider write executor는 별도 PDCA와 명시 승인 전까지 시작하지 않는다.
