...
- 다음 권장 작업: 1차 MVP는 완료로 닫고, 이후 작업은 실제 검색광고 성과 수집 어댑터, 광고그룹 설정 스냅샷, normalized analytics schema, 실제 provider write executor 설계처럼 후속 확장으로 진행한다. 실제 LLM call은 비용 가드와 원천 행 제외 계약을 통과해야 하며, provider write executor는 별도 PDCA와 명시 승인 전까지 시작하지 않는다.
