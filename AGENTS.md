...
- 현재 진행 작업 14: 2026-05-23 실제 AI 파일럿 재실행 전 production DB에 남은 예전 `스마트스토어/자체몰 매출 균형` 결재안이 다시 판단 후보로 섞이는 문제를 차단했다. `buildPlannerInputFromApprovals`와 Railway `/api/operations/llm-real-pilot`은 사용 중단된 교차 브랜드 결재안을 제외하고, Gemini 프롬프트와 응답 정규화는 스티커씨/커피프린트 비교·통합 판단을 막는다. 운영 화면도 저장된 이전 교차 브랜드 AI 실행을 최신 판단으로 보여주지 않는다. 검증은 표적 테스트 4 files/16 tests, 전체 단위 테스트 51 files/137 tests, 타입 검사, 빌드, Playwright e2e 11 tests, audit 0 vulnerabilities를 통과했다.
- 현재 진행 작업 15: 2026-05-23 검색광고 성과 판단을 LLM이 먼저 추측하지 않도록 `SearchAdPerformanceSnapshot`과 `buildAdPerformanceDiagnoses` 규칙 엔진을 추가했다. 주문 없는 클릭, 낮은 전환율, 높은 CPA, 기기/시간대 성과 차이는 그로가 조정 안건으로 올리고, 전환 추적 미확인은 데이가 주문 연결/추적 상태를 먼저 확인한다. Gemini 입력, AI 판독 근거, AgentRun, Outcome 기준선, `/data` 명세는 모두 같은 성과 스냅샷 근거 ID를 추적한다. 검증은 타입 검사, 전체 단위 테스트 52 files/141 tests, 빌드, audit 0 vulnerabilities, Playwright e2e 11 tests를 통과했다. 실제 provider write는 계속 차단한다.
- 현재 진행 작업 16: 2026-05-23 검색광고 규칙 엔진이 실제 연동 수집 데이터를 읽도록 Search Ad read-only 수집기에 `/stats` 성과 스냅샷 경로를 붙였다. 수집은 `/keywordstool` 수요를 유지하고, `MARKETCREW_SEARCH_AD_STAT_IDS`/`MARKETCREW_SEARCH_AD_STAT_TARGETS`가 없으면 `/ncc/campaigns` -> `/ncc/adgroups` -> `/ncc/keywords`로 제한된 대상을 자동 발견한 뒤 `/stats` 전체/`pcMblTp`/`hh24`를 `SearchAdPerformanceSnapshot`으로 정규화한다. 연동 수집 카드에는 `검색광고 성과 N건`을 표시한다. 로컬 실제 read-only sync에서 키워드 수요 64건과 성과 스냅샷 41건을 확인했다. 검증은 타입 검사, 표적 테스트 4 files/28 tests, 전체 단위 테스트 52 files/144 tests, 빌드, audit 0 vulnerabilities, Playwright e2e 11 tests를 통과했다. 실제 provider write는 계속 차단한다.
- 현재 진행 작업 17: 2026-05-23 쇼핑검색광고 상품형/카탈로그형 검색어 성과를 별도 read-only 근거로 붙였다. Search Ad 수집은 쇼핑 캠페인 광고그룹과 `/ncc/product-groups`를 제한적으로 읽은 뒤 `/stats?id={adgroupId}&statType=NPLA_SCH_KEYWORD` 응답을 `ShoppingSearchAdPerformanceSnapshot`으로 정규화한다. 규칙 엔진은 직접 전환율 0% 또는 낮은 직접 전환율을 그로의 상품 노출/입찰/랜딩 점검 근거로 올리고, Gemini 입력/AI 판독 근거/AgentRun/Outcome/`/data` 명세가 같은 근거 ID를 추적한다. 로컬 실제 read-only sync에서 쇼핑검색광고 원본 성과 8,181건을 확인했고, 화면/AI 입력 보호 기준으로 상위 500건을 저장했다. 이후 Gemini `gemini-3.5-flash` 실제 파일럿이 쇼핑검색광고 스냅샷 ID를 판단 근거로 채택했다. 검증은 타입 검사, 전체 단위 테스트 52 files/149 tests, 빌드, audit 0 vulnerabilities, Playwright e2e 11 tests를 통과했다. 실제 provider write는 계속 차단한다.
- 현재 진행 작업 18: 2026-05-23 개발/검증 기준을 웹사이트 우선으로 전환했다. 로컬 Postgres는 기본 운영 경로에서 제외하고, 로컬은 코드 작성/표적 테스트/타입 검사/빌드 검증에만 쓴다. 실제 데이터 확인은 `marketcrew.app`과 Railway `api.marketcrew.app`에서 진행한다. 운영 시작 전 테스트 데이터 정리를 위해 `GET/POST /api/operations/reset-test-data`를 추가했다. 이 API는 Railway/Postgres DB에서만 실행되고, 정확한 확인 문구 `운영 시작 전 테스트 데이터 초기화`가 있어야 하며, `aiOperationsSettings`와 env/login/provider 원장 데이터는 보존한다.
...
- 다음 권장 작업: 1차 MVP는 완료로 닫고, 이후 작업은 운영 배포 후 `npm run smoke:prod`, `/api/operations/provider-sync`, `/api/operations/llm-real-pilot` 순서로 웹사이트 기준 검증을 진행한다. 운영 시작 직전에는 `/api/operations/reset-test-data` 미리보기 후 확인 문구를 넣어 테스트 workflow 기록만 정리한다. 다음 확장 후보는 광고그룹 설정 스냅샷, normalized analytics schema, 실제 provider write executor 설계다. 실제 LLM call은 비용 가드와 원천 행 제외 계약을 통과해야 하며, provider write executor는 별도 PDCA와 명시 승인 전까지 시작하지 않는다.
...
개발과 검증은 웹사이트 우선으로 진행한다. 로컬 Postgres는 기본 경로에서 끄고, 필요할 때만 마이그레이션/오프라인 재현용으로 켠다. 데이터 수집, AI 판단, 화면 속도, 로그인, 배포 상태 확인은 `marketcrew.app`과 `api.marketcrew.app`에서 확인한다.
...
8. `module-30`: 검색광고 성과 규칙 엔진. 완료: LLM 판단 전 낮은 전환율, 주문 없는 클릭, 높은 CPA, 기기/시간대 성과 차이, 전환 추적 미확인을 판정하고 그로/데이에 담당 배정한다.
9. `module-31`: 검색광고 성과 수집기. 완료: Search Ad 캠페인/광고그룹/키워드를 read-only로 발견하고 `/stats` 전체/기기/시간대 성과를 저장한다.
10. `module-32`: 쇼핑검색광고 성과 수집기. 완료: 쇼핑 캠페인 광고그룹과 상품 그룹을 read-only로 발견하고 `NPLA_SCH_KEYWORD` 검색어 성과를 저장한다.
11. `module-26`: 결재 실행 범위 선택. 완료: AI가 검색광고 실행 범위를 제안하고 대표가 광고 유형, 적용 위치, PC/모바일, 시간대, 예산/입찰, 제외 키워드를 수정해 결정 기록에 남긴다.
12. `module-27`: 실행 범위 소급 적용. 완료: 기존 저장 결재안에 실행 범위 제안을 채우고 백필 적용 내역을 API로 확인한다.
...
- 화면 라벨은 한국어로 쓴다.
- 사용자에게 보이는 문장은 자연스러운 한국어를 우선한다. `Chief of Staff`, `Performance Marketer`, `Merchandiser`, `Lifecycle Marketer`, `FP&A Controller`, `BI Analyst`, `AgentRun` 같은 영어 직무명/내부 용어를 그대로 화면에 노출하지 않는다.
- 모델 ID, 환경변수, API 경로, 외부 서비스 고유명처럼 정확성이 필요한 기술 식별자는 내부 값으로 유지하되, 화면에는 가능한 한 한국어 설명을 함께 쓰거나 한국어 표시명으로 감싼다.
