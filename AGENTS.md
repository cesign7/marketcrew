...
- 현재 진행 작업 6: 2026-05-22 속도 개선 후속으로 Railway 백엔드 API 1차 분리 코드를 추가했다. `services/marketcrew-api`는 Railway Postgres와 가까운 곳에서 `workflow_records`를 읽고 `GET /api/workflow-state`로 캐시된 workflow state를 제공한다. Vercel 화면 loader는 `MARKETCREW_BACKEND_API_URL`이 있으면 Railway API를 먼저 사용하고, 실패하면 기존 Postgres 직접 읽기로 fallback한다. 실제 provider write는 계속 차단한다. Railway production 서비스는 `marketcrew-api`, 공개 URL은 `https://marketcrew-api-production.up.railway.app`이다.
- 클라우드 연결 상태: GitHub `https://github.com/cesign7/marketcrew`의 기본 브랜치는 `main`이며 현재 MVP 커밋이 올라가 있다. Vercel production은 `https://marketcrew.vercel.app`이고 Git 연결의 production branch도 `main`이다. Railway project는 `marketcrew`, Postgres service는 `Online`, API service는 `marketcrew-api`다. Vercel production은 Railway Postgres를 `MARKETCREW_REPOSITORY_MODE=db`로 읽고, `MARKETCREW_BACKEND_API_URL`, `MARKETCREW_BACKEND_API_TOKEN`, `MARKETCREW_BACKEND_API_TIMEOUT_MS`가 있으면 Railway API read model을 우선 사용한다. Railway 쪽 API 토큰 키는 `MARKETCREW_API_TOKEN`이다. `/api/operations/workflow-state` production smoke에서 `approvalRequests=5`, `providerSyncReports=18`, `agentRuns=1`을 확인했다. Custom domain `marketcrew.app`과 `www.marketcrew.app`은 Vercel project에 추가됐고 Cloudflare DNS는 `A @ 76.76.21.21`, `A www 76.76.21.21`, DNS only로 설정됐다. 공개 앱은 `MARKETCREW_AUTH_SECRET`, `MARKETCREW_OWNER_PASSWORD_HASH` 기반 대표 로그인으로 보호한다. 기존 GitHub branch `feat-ai-marketing-operations-mvp`는 아직 남아 있으며 삭제는 명시 지시 전까지 하지 않는다.
...
운영 확장 단계에서는 Vercel을 화면과 로그인 게이트 중심으로 두고, Railway 백엔드 API를 DB 근접 read/write, provider 수집, LLM 분석, 스케줄러, 승인 후 실행 처리의 중심으로 둔다. 첫 분리 기준은 `services/marketcrew-api`의 workflow-state read model이며, 프론트 loader는 Railway API 실패 시 기존 DB 직접 읽기로 돌아가야 한다.
