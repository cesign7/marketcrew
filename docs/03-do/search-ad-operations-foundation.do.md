...
### follow-up 실행 안전장치와 이력 보강

- 후보 카드, 규칙 상세, 캠페인/광고그룹 상태 토글에 최종 확인 단계를 추가했다.
- 실제 write gate가 열려 있으면 최종 실행 전 “실제 네이버 광고 상태를 변경합니다” 문구를 다시 보여준다.
- 실행 이력은 캠페인/광고그룹/키워드 대상 종류, 요청 방향, 생성/이력 시간을 함께 보여준다.
- provider 응답이 성공했는데 별도 오류 문구가 없는 실행 로그도 `네이버 검색광고 키워드를 껐습니다`처럼 사람이 읽는 사유로 복원한다.

### follow-up 후보 처리 흐름과 긴 상품명 표시

- 규칙 결과 상세 화면에 `후보 처리 흐름`을 추가했다.
- 제외어, 키워드 추가, 상품 확장, 소재·랜딩, 입찰, 타게팅, 전환 점검 후보별로 첫 실행 대표 승인, 반복 위임 여부, 실행 전 확인 단계를 보여준다.
