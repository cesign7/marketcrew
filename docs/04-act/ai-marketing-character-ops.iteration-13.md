...
| Cache safety | production에 남은 예전 view model 캐시처럼 새 계획 필드가 없어도 화면이 깨지지 않게 빈 목록으로 처리한다. |
...
| 대상 컴포넌트/view model 테스트 | 3 files, 6 tests passed |
| 데이터 필터 테스트 | `data-filters` passed. 검색광고와 데이터랩은 공통 마케팅 근거로 유지된다. |
| 전체 단위 테스트 | 37 files, 104 tests passed |
