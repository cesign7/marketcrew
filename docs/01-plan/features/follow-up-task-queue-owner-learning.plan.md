...
| **Function/UX Effect** | 대표는 `/follow-ups`에서 모아, 데이 등 담당자별 열린 업무를 보고 완료 처리하며, 다음 추천에 반영될 결정 기준을 확인한다. |
...
| FR-02 | 담당 캐릭터별 queue를 만든다. | 모아/그로/프로/카피/리피/마루/데이 별 open/done count와 task list가 보인다. |
