...
| `ADEXTENSION` | 확장소재광고 성과 보고서 | 15 | 확장소재 노출/클릭/비용 | 파워링크 확장소재 점검. 쇼핑검색광고 확장소재는 전환 보고서 연결 전까지 비효율 판단에서 제외 |
| `ADEXTENSION_CONVERSION` | 확장소재 전환 보고서 | 확인 필요 | 확장소재 전환/매출 | 현재 수집 대상 아님. 첫 다운로드 파일 컬럼 확정 후 정규화와 규칙 합산 연결 |
