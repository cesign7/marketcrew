...
- [ ] 캠페인/광고그룹 ON/OFF를 미리보기와 실행 로그를 거쳐 처리한다.
- [ ] 키워드 ON/OFF는 데이터 모델과 UI에는 준비하되, 첫 실행 범위는 캠페인/광고그룹으로 제한한다.
...
| FR-13 | 캠페인/광고그룹 ON/OFF 실행 전 변경 전/후 diff와 예상 영향 범위를 보여준다. | High | Pending |
| FR-14 | ON/OFF 실행은 write gate, 대표 확인, 실행 로그, 실패 로그를 남긴다. | High | Pending |
