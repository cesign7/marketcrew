...
| **Solution** | `/operations`와 `/settings`에 LLM 비용 가드를 추가해 provider/key readiness, 공식 모델 가격 기준, env 원화 단가, 예상 token, 1회/일 예산, 누적 AgentRun 비용, privacy gate를 한 번에 보여준다. |
...
| **RISK** | 공식 가격 기준이 오래되거나, key가 있다는 이유만으로 live call을 열거나, 원천 row가 LLM 입력에 섞일 수 있다. |
...
7. 공식 가격표에서 확인한 USD 단가를 reference로 보여주되 실제 차단 계산은 운영 env 원화 단가를 사용한다.
...
| 실시간 vendor 가격/환율 자동 반영 | 공식 가격 기준은 확인일을 표시한 reference로 두고, 운영 계산은 env 원화 단가로 관리한다. |
...
| FR-07 | 공식 모델 가격 기준을 보여준다. | 설정된 Gemini 모델의 공식 USD 입력/출력/캐시 단가와 확인일이 보인다. |
...
| SC-4 | `/operations`와 `/settings`에서 비용 가드 패널이 보인다. |
| SC-5 | unit/typecheck/build/e2e/browser smoke가 통과한다. |
| SC-6 | 실제 LLM 호출과 외부 provider write는 열리지 않는다. |

## Next Step

Iteration 1은 `module-6 LLM 인터페이스`의 실제 호출 전 비용 governance를 완성한다. Iteration 2는 같은 모듈에서 공식 모델 가격 기준을 설정 화면에 반영한다. 이후 별도 cycle에서 live LLM adapter를 붙이더라도 이 gate를 먼저 통과해야 한다.
