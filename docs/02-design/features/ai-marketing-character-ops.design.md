...
운영 기준은 자동 스케줄과 수동 연동 수집을 병행한다. 자동 스케줄은 기준 데이터를 빠짐없이 쌓기 위한 것이고, 수동 수집은 방금 바뀐 광고/상품/CRM 또는 결재 직전 최신 근거를 보강하기 위한 것이다. 같은 채널/브랜드/날짜/기간/시즌 이벤트의 데이터는 최신 스냅샷으로 갱신하되, 수집 성공/실패 이력은 별도 보관한다.
