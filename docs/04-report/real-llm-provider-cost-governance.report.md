...
실제 LLM 호출을 켜기 전에 비용과 token 안전장치를 운영실에서 확인하는 slice를 완료했다.

| Area | Result |
|------|--------|
| View model | `buildLlmCostGovernanceView` 추가 |
| UI | `/operations`에 `LLM 비용 가드` 패널 추가 |
| API | `/api/operations/llm-cost-governance` read-only route 추가 |
| Tests | unit 3개, agenda room assertion, e2e smoke 추가 |
| Docs | plan/design/do/act/check/qa/report 작성 |

## Operator Behavior

현재 local env 상태에서는 provider/key는 준비되어 있지만 비용 단가와 예산 env가 없기 때문에 운영실은 `live call 차단`을 보여준다. 이는 의도된 안전 상태다. deterministic fallback은 계속 동작한다.

## Changed Files

| Type | Files |
|------|-------|
| Feature | `src/features/agenda-room/buildLlmCostGovernanceView.ts`, `src/features/agenda-room/types.ts`, `src/features/agenda-room/buildAgendaRoomViewModel.ts` |
...
| Docs | `docs/01-plan/features/real-llm-provider-cost-governance.plan.md`, `docs/02-design/features/real-llm-provider-cost-governance.design.md`, `docs/03-do/real-llm-provider-cost-governance.do.md`, `docs/04-act/real-llm-provider-cost-governance.iteration-1.md`, `docs/03-analysis/real-llm-provider-cost-governance.analysis.md`, `docs/05-qa/real-llm-provider-cost-governance.qa-report.md`, `docs/04-report/real-llm-provider-cost-governance.report.md` |
...
| Risk | Status |
|------|--------|
| 실제 LLM 비용 단가/예산 정책 미확정 | 대표가 env 정책값을 정하면 재검증 필요 |
