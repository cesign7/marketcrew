...
| Docs | `docs/01-plan/features/real-llm-provider-cost-governance.plan.md`, `docs/02-design/features/real-llm-provider-cost-governance.design.md`, `docs/03-do/real-llm-provider-cost-governance.do.md`, `docs/04-act/real-llm-provider-cost-governance.iteration-1.md`, `docs/04-act/real-llm-provider-cost-governance.iteration-2.md`, `docs/04-act/real-llm-provider-cost-governance.iteration-3.md`, `docs/04-act/real-llm-provider-cost-governance.iteration-4.md`, `docs/03-analysis/real-llm-provider-cost-governance.analysis.md`, `docs/05-qa/real-llm-provider-cost-governance.qa-report.md`, `docs/04-report/real-llm-provider-cost-governance.report.md` |
...
## 2026-05-23 AI Operations Settings Follow-up

`/people` 인사과와 `/settings`에서 저장한 AI 예산/모델 설정을 실제 비용 가드 계산에 연결했다.

| Area | Result |
|------|--------|
| 저장 설정 | `AiOperationsSettings`를 agenda room view model에서 읽어 `buildLlmCostGovernanceView`에 전달 |
| 비용 계산 | env 원화 단가가 없으면 저장된 환율과 공식 USD 가격표로 원화 예상 비용 계산 |
| 예산 게이트 | 저장된 월/일/1회 예산과 입력/출력/총 토큰 상한을 gate에 반영 |
| 월 예산 | 이번 달 누적, 월 예산, 월 잔여 예산을 운영실 비용 가드에 표시 |
| 검증 | `npm run typecheck`, `npm test -- --run`, `npm run build`, 비용 가드/네비게이션 Playwright smoke 통과 |

1차 MVP 대비 진행율은 100%로 유지한다. 이번 작업은 live LLM 호출을 여는 것이 아니라 호출 전 예산 판단의 설정 연결을 완성한 것이다.

## 2026-05-23 Settings Access Fix

인사과와 설정 화면이 hosted Vercel에서 전체 workflow-state 응답에 의존하던 문제를 수정했다.

| Area | Result |
|------|--------|
| 원인 | `/settings`, `/people`이 AI 설정만 필요해도 `/api/operations/workflow-state` 전체 응답을 요구했고, 운영 full state fetch 실패 시 페이지 렌더링이 중단됨 |
| 조치 | `/api/settings/ai-operations`가 `settings`와 `peopleOfficeView`를 반환하게 하고 두 화면은 이 작은 전용 응답을 사용 |
| 저장 후 반영 | AI 설정 저장 성공 시 운영실 view model cache를 비워 비용 가드 갱신을 앞당김 |
| 검증 | `npm run typecheck`, `npm test -- --run`, `npm run build`, navigation Playwright smoke 통과 |
