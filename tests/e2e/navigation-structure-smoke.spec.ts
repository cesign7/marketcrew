...
test("대표 업무실은 왼쪽 업무 메뉴와 상단 기준 필터로 나뉜다", async ({ page }) => {
  ...
  await expect(navigation.getByRole("link", { name: "캐릭터 업무데스크" })).toHaveCount(0);
...
test("인사과 메뉴에서 캐릭터 롤모델과 월별 AI 사용 명세를 확인한다", async ({ page }) => {
  ...
  await expect(page.getByText("키워드 성과/추천에 필요한 캐릭터부터 활성화")).toBeVisible();
  ...
  await page.reload();
  await expect(page.getByLabel("롤모델").first()).toHaveValue(/대표 비서실장처럼 안건 우선순위와 비용 위험/);
  await expect(page.getByLabel("롤모델").first()).toHaveValue(/대표 첫 승인/);
});
...
test("왼쪽 캐릭터별 데스크에서 캐릭터별 업무 화면으로 이동한다", async ({ page }) => {
  ...
  await expect(page.getByRole("region", { name: "쇼핑검색광고 검색어" })).toBeVisible();
  await expect(page.getByRole("region", { name: "추천 키워드 근거" })).toBeVisible();
